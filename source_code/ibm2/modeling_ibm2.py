...
class GammaIB4dt(nn.Module):
    def __init__(self, hidden_size, alphas=None, max_seqlen=4096, **kwargs) -> None:
        super().__init__()
        self.alphas = alphas
    ...
    def compute_loss(self, param_alphas, epsilon=1e-6):
        if self.alphas is None:
            alphas = self.init_alphas(param_alphas)  # length is the second dimension of att
        ...
        if self.alphas.size(0) >= param_alphas.size(1):
            alphas = self.alphas[-param_alphas.size(1):] # [-att.size(1):]
        else:
...
class BernoulliIB4dA(nn.Module):
    def __init__(self, hidden_size, temp=1, thetas=None, max_seqlen=4096, **kwargs) -> None:
        super().__init__()
        self.epoch_frac = 0
    ...
    def init_thetas(self, attn):
        ...
        # decay_factor = torch.flip(decay_factor, dims=[0])
    ...
    def compute_loss(self, att, epsilon=1e-6):
        ...
        reg_loss = (att * torch.log(att / thetas + epsilon) +
                    (1 - att) * torch.log((1 - att) / (1 - thetas + epsilon) + epsilon))
        
        return reg_loss.mean(dim=-1)#.mean()
...
class IBM2Mixer(nn.Module):
    ...
    def __init__(self, config: IBM2Config, layer_idx: int):
        super().__init__()
        ...
        IB_class = BernoulliIB4dA if self.ib_type == 'bernoulli' else GammaIB4dt if self.ib_type == 'gamma' else None
        default_ib_layers = [0, config.num_hidden_layers // 2 - 1, config.num_hidden_layers - 1]
        ib_layers = getattr(config, 'ib_layers', default_ib_layers)
        self.ib = IB_class(self.num_heads, max_seqlen=config.max_seq_length) if layer_idx in ib_layers else None
    ...
    def cuda_kernels_forward(
        ...
        if cache_params is not None and cache_position is not None and cache_position[0] > 0:
            ...
            # assert False, "The single step cache path is not implemented for IBM2 yet."
    ...
    def forward(
        ...
        if attention_mask is not None and attention_mask.shape[1] > 1 and attention_mask.shape[0] > 1:
            ...
            hidden_states = (hidden_states * attention_mask[:, :, None]).to(dtype)
        assert False, "The torch_forward is not implemented for IBM2 yet."
        return self.torch_forward(hidden_states, cache_params, cache_position, attention_mask)


#! Note: IB4dA is only applied at layers 0, 23, and 47 for IBM2
class IBM2Block(nn.Module):
    def __init__(self, config, layer_idx):
        super().__init__()
        #! inside operation of IB4dA, so comment it out
        self.config = config
        self.layer_idx = layer_idx
...
class IBM2PreTrainedModel(PreTrainedModel, GenerationMixin):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = IBM2Config
    base_model_prefix = "backbone"
    _no_split_modules = ["IBM2Block"]
...
class IBM2CausalLMOutput(ModelOutput):
    """
...
class IBM2Model(IBM2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.embeddings = nn.Embedding(config.vocab_size, config.hidden_size)
        self.layers = nn.ModuleList([IBM2Block(config, layer_idx=idx) for idx in range(config.num_hidden_layers)])
...
class IBM2ForCausalLM(IBM2PreTrainedModel):
    _tied_weights_keys = []

    def __init__(self, config):
        super().__init__(config)
        self.backbone = IBM2Model(config)
    ...
    def forward(
        ...
        **kwargs,  # for now we need this for generation
    ) -> Union[Tuple, IBM2CausalLMOutput]:
        r"""
        ...
        if labels is not None:
            ...
            #! HACK loss collection for IBS
            auxiliary_loss = []
            # mask = (labels != criterion.ignore_index).float()
            def collect_auxiliary_loss(module):
                if hasattr(module, "get_auxiliary_loss"):
                    aux = module.get_auxiliary_loss()  # shape: [batch, seq_len]
                    if aux.shape[:2] == labels.shape:  # 对齐 labels
                        mask = (labels != criterion.ignore_index).float()
                        aux = aux * mask  # 在 -100 位置归零
        ...
        return IBM2CausalLMOutput(
            loss=loss,
            kl_loss=kl_loss if labels is not None else None,
            logits=logits,
